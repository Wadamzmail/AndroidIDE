...
import dev.mutwakil.androidide.activities.FolderPickerActivity
import dev.mutwakil.androidide.activities.IDEConfigurations
import dev.mutwakil.androidide.databinding.DialogAtcWizardBinding
...
import dev.mutwakil.androidide.templates.android.etc.NativeCpp.Check
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main) {
                ...
                when {
                    !hasNdk -> showNdkError(
                        ctx,
                        getString(R.string.error_ndk_not_found_or_incompatible)
                    )
    ...
    private fun createProject(ctx: Context) {
        ...
        val pkg =
            binding.packageNameInput.text?.toString()?.trim().takeUnless { it.isNullOrBlank() }
                ?: "com.example.${
                    selectedTemplate?.displayName?.replace(" ", ".")?.lowercase() ?: "myproject"
                }"
        ...
        val minSdkDisplay = sdkValues.map { it.displayName() }.toTypedArray()
        val selectedIdx =
            minSdkDisplay.indexOf(binding.minSdkInput.text?.toString()).coerceAtLeast(0)
        ...
        MaterialAlertDialogBuilder(ctx)
            ...
            .setSingleChoiceItems(versionLabels, 0) { dialog, which ->
                ...
                Check.validateCMakeVersion(selectedVersion)?.let { path ->
                    ...
                    Toast.makeText(ctx, "CMake $selectedVersion selected", Toast.LENGTH_SHORT)
                        .show()
                    dialog.dismiss()
                } ?: Toast.makeText(ctx, "Invalid CMake: $selectedVersion", Toast.LENGTH_SHORT)
                    .show()
            }
...
internal object SafResolver {
    ...
    private const val ANDROIDIDE_DOCS_AUTHORITY = "dev.mutwakil.androidide.documents"
