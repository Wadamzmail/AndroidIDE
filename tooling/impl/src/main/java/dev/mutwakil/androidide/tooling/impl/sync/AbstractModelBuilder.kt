...
import dev.mutwakil.androidide.builder.model.IDESyncIssue
import dev.mutwakil.androidide.tooling.api.util.ToolingProps
import dev.mutwakil.androidide.utils.AndroidPluginVersion
import dev.mutwakil.androidide.utils.AndroidPluginVersion.Companion.MINIMUM_SUPPORTED
import dev.mutwakil.androidide.utils.ILogger
import dev.mutwakil.androidide.utils.StacktraceUtils
...
abstract class AbstractModelBuilder<P, R> : IModelBuilder<P, R> {
    companion object {
         ...
         */
        @JvmStatic
        protected fun getAndroidVersions(
            model: Model,
            controller: BuildController,
        ): Versions? = controller.findModel(model, Versions::class.java)
         ...
         */
        @JvmStatic
        protected fun <T> BuildController.getModelAndLog(
            target: Model,
            modelType: Class<T>,
        ): T {
            return withStopWatch(modelType) {
         ...
         * [ generateMessage(Object...)][dev.mutwakil.androidide.utils.ILogger.generateMessage] in [ILogger][dev.mutwakil.androidide.utils.ILogger].
         *
        ...
        protected fun generateMessage(vararg objects: Any?): String {
            ...
            for (msg in objects) {
                ...
                sb.append(
                    if (msg is Throwable) {
                        StacktraceUtils.getFullStackTrace(
                            msg as Throwable?,
                        )
                    } else {
                        msg
