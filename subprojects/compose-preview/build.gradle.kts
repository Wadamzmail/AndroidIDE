...
/*
 * Keep generated outputs of the custom tasks separate.
 *
 * compose-compiler/ -> Compose compiler plugin
 * compose-jars/     -> extracted Compose runtime JARs
 * compose-dex/      -> compiled Compose runtime DEX
 */
val composeCompilerOutput =
    layout.buildDirectory.dir("compose-compiler")

val composeRuntimeOutput =
    layout.buildDirectory.dir("compose-jars")

val composeDexOutput =
    layout.buildDirectory.dir("compose-dex")
...
val extractComposeClasses by tasks.registering {
    ...
    val outputDir = composeRuntimeOutput

    /*
     * Inputs:
     *   Compose AAR/JAR dependencies
     *
     * Outputs:
     *   Extracted runtime JARs
     */
...
fun resolveAndroidJar(): File {
    ...
    return platformsDir.listFiles()
        ...
        ?.sortedByDescending { it.name }
        ?.firstNotNullOfOrNull {
            File(it, "android.jar").takeIf { jar ->
                jar.exists()
            }
        }
        ?: throw GradleException(
...
val compileRuntimeDex by tasks.registering {
    ...
    val jarsDir = composeRuntimeOutput
    val dexOutputDir = composeDexOutput

    /*
     * Inputs:
     *   Extracted Compose runtime JARs
     *
     * Outputs:
     *   Generated Compose runtime DEX
     */
    inputs.dir(jarsDir)
    ...
    doLast {
        val outDir =
            dexOutputDir.get().asFile.apply {
                mkdirs()
            }

        val runtimeJars =
            jarsDir.get().asFile.listFiles { file: File ->
                file.extension == "jar"
            }?.toList()
                ?: throw GradleException(
                    "No runtime JARs found to compile to DEX"
                )
