package dev.mutwakil.androidide.compose.preview.data.repository

import android.content.Context
import dev.mutwakil.androidide.compose.preview.compiler.CompileDiagnostic
import dev.mutwakil.androidide.compose.preview.compiler.CompilerDaemon
import dev.mutwakil.androidide.compose.preview.compiler.ComposeClasspathManager
import dev.mutwakil.androidide.compose.preview.compiler.ComposeCompiler
import dev.mutwakil.androidide.compose.preview.compiler.ComposeDexCompiler
import dev.mutwakil.androidide.compose.preview.compiler.DexCache
import dev.mutwakil.androidide.compose.preview.data.source.ProjectContext
import dev.mutwakil.androidide.compose.preview.data.source.ProjectContextSource
...
class ComposePreviewRepositoryImpl(
    ...
    private var runtimeDex: List<File> = emptyList()
    ...
    override suspend fun initialize(
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                runtimeDex = cpManager.getOrCreateRuntimeDexFiles()

                if (runtimeDex.isEmpty()) {
                ...
                LOG.info("Compose runtime DEX ready: {} files", runtimeDex.size)
                ...
                LOG.info("Repository initialized, runtimeDex={} files", runtimeDex.size)
    ...
    private fun <T> requireInitialized(value: T?, name: String): T {
        return value
            ?: throw IllegalStateException(
                "Repository not initialized: $name is null. Call initialize() first."
            )
    }
    ...
    override suspend fun compilePreview(
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                val compiler =
                    requireInitialized(this@ComposePreviewRepositoryImpl.compiler, "compiler")
                val compilerDaemon = this@ComposePreviewRepositoryImpl.compilerDaemon
                val dexCompiler =
                    requireInitialized(this@ComposePreviewRepositoryImpl.dexCompiler, "dexCompiler")
                val workDir =
                    requireInitialized(this@ComposePreviewRepositoryImpl.workDir, "workDir")
                val classpathManager =
                    requireInitialized(
                        this@ComposePreviewRepositoryImpl.classpathManager,
                        "classpathManager",
                    )
                ...
                if (cached != null) {
                    LOG.info(
                        "Using cached DEX for hash: {}, runtimeDex={}, projectDexFiles={}",
                        sourceHash,
                        runtimeDex.size,
                        context.projectDexFiles.size,
                    )
                ...
                LOG.info(
                    "Using {} project classpaths for compilation",
                    context.compileClasspaths.size,
                )

                val classpath =
                    cachedClasspath
                        ?: classpathManager
                            .getCompilationClasspath(context.compileClasspaths)
                            .also {
                                cachedClasspath = it
                            }
                ...
                if (compilerDaemon != null) {
                    val daemonResult =
                        try {
                            ...
                            LOG.warn(
                                "Daemon compilation failed, falling back to regular compiler",
                                e,
                            )
                            null
                        }
                ...
                LOG.info(
                    "Preview ready: {} with {} previews, {} project DEX files",
                    fullClassName,
                    parsedSource.previewConfigs.size,
                    context.projectDexFiles.size,
                )
