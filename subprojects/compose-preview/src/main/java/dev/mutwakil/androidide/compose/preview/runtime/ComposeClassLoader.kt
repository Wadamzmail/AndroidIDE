package dev.mutwakil.androidide.compose.preview.runtime

import android.content.Context
import dalvik.system.DexClassLoader
import java.io.File
import org.slf4j.LoggerFactory

class ComposeClassLoader(private val context: Context) {

    private var currentLoader: DexClassLoader? = null
    private var currentCacheKey: String? = null
    private var runtimeDexFiles: List<File> = emptyList()
    private var projectDexFiles: List<File> = emptyList()

    fun setRuntimeDexFiles(runtimeDexFiles: List<File>) {
        val existingFiles = runtimeDexFiles.filter { it.exists() }

        LOG.info(
            "setRuntimeDexFiles called: {} files ({} exist)",
            runtimeDexFiles.size,
            existingFiles.size,
        )

        this.runtimeDexFiles = existingFiles
        release()

        existingFiles.forEach {
            LOG.info("  Runtime DEX: {}", it.absolutePath)
        }
    }

    fun setProjectDexFiles(dexFiles: List<File>) {
        val existingFiles = dexFiles.filter { it.exists() }

        LOG.info(
            "setProjectDexFiles called: {} files ({} exist)",
            dexFiles.size,
            existingFiles.size,
        )

        projectDexFiles = existingFiles
        release()

        existingFiles.forEach {
            LOG.info("  Project DEX: {}", it.absolutePath)
        }
    }

    fun loadClass(
        dexFile: File,
        className: String,
    ): Class<*>? {
        if (!dexFile.exists()) {
            LOG.error(
                "DEX file not found: {}",
                dexFile.absolutePath,
            )
            return null
        }

        return try {
            val loader = getOrCreateLoader(dexFile)

            loader.loadClass(className).also {
                LOG.debug("Loaded class: {}", className)
            }
        } catch (e: ClassNotFoundException) {
            LOG.error(
                "Class not found: {}",
                className,
                e,
            )
            null
        } catch (e: Exception) {
            LOG.error(
                "Failed to load class: {}",
                className,
                e,
            )
            null
        }
    }

    private fun getOrCreateLoader(dexFile: File): DexClassLoader {
        val dexFiles = mutableListOf<File>()

        dexFiles.add(dexFile)
        dexFiles.addAll(projectDexFiles)
        dexFiles.addAll(runtimeDexFiles)

        val cacheKey = buildCacheKey(dexFiles)

        val dexPath =
            dexFiles.joinToString(File.pathSeparator) {
                it.absolutePath
            }

        LOG.info(
            "getOrCreateLoader: runtimeDexFiles={}, " + "projectDexFiles={}, totalDexFiles={}",
            runtimeDexFiles.size,
            projectDexFiles.size,
            dexFiles.size,
        )

        if (currentCacheKey == cacheKey && currentLoader != null) {
            LOG.debug("Reusing existing DexClassLoader")
            return currentLoader!!
        }

        release()

        val optimizedDir =
            File(
                context.codeCacheDir,
                "compose_preview_opt",
            )

        optimizedDir.deleteRecursively()
        optimizedDir.mkdirs()

        val loader =
            DexClassLoader(
                dexPath,
                optimizedDir.absolutePath,
                null,
                context.classLoader,
            )

        currentLoader = loader
        currentCacheKey = cacheKey

        LOG.info(
            "Created new DexClassLoader with {} DEX files: {}",
            dexFiles.size,
            dexPath,
        )

        return loader
    }

    private fun buildCacheKey(dexFiles: List<File>): String {
        return dexFiles.joinToString("|") { file ->
            "${file.absolutePath}:${file.lastModified()}"
        }
    }

    fun release() {
        currentLoader = null
        currentCacheKey = null

        val optimizedDir =
            File(
                context.codeCacheDir,
                "compose_preview_opt",
            )

        if (optimizedDir.exists()) {
            optimizedDir.deleteRecursively()
        }

        LOG.debug("Released ComposeClassLoader resources")
    }

    companion object {
        private val LOG = LoggerFactory.getLogger(ComposeClassLoader::class.java)
    }
}
