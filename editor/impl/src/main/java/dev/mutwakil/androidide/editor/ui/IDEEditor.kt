...
import dev.mutwakil.androidide.editor.language.IDELanguage
import dev.mutwakil.androidide.editor.language.cpp.CppLanguage
import dev.mutwakil.androidide.editor.language.groovy.GroovyLanguage
import dev.mutwakil.androidide.editor.language.treesitter.TreeSitterLanguage
...
import dev.mutwakil.androidide.editor.snippets.AbstractSnippetVariableResolver
import dev.mutwakil.androidide.editor.snippets.FileVariableResolver
import dev.mutwakil.androidide.editor.snippets.WorkspaceVariableResolver
import dev.mutwakil.androidide.editor.ui.IDEEditor.Companion.LARGE_FILE_LINE_THRESHOLD
import dev.mutwakil.androidide.eventbus.events.editor.ChangeType
import dev.mutwakil.androidide.eventbus.events.editor.ColorSchemeInvalidatedEvent
...
import dev.mutwakil.androidide.syntax.colorschemes.DynamicColorScheme
import dev.mutwakil.androidide.syntax.colorschemes.SchemeAndroidIDE
import dev.mutwakil.androidide.tasks.JobCancelChecker
...
open class IDEEditor @JvmOverloads constructor(
    ...
    private var fileVersion = AtomicInteger(0)
        ...
        createLanguage(file) { language ->
            ...
            if (language is TreeSitterLanguage) {
                IDEColorSchemeProvider.readSchemeAsync(
                    context = context, coroutineScope = editorScope,
                    type = extension
                ) { scheme ->
    ...
    open fun applyTreeSitterLang(
        language: TreeSitterLanguage, type: String,
    ...
    protected open fun dispatchDocumentChangeEvent(event: ContentChangeEvent) {
        ...
        val changeEvent = DocumentChangeEvent(
            file, changedText, text.toString(), fileVersion.incrementAndGet(), type,
    ...
    private fun configureFlashbar(
        builder: Flashbar.Builder, @StringRes message: Int,
