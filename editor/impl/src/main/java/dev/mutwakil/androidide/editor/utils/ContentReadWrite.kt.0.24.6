...
package dev.mutwakil.androidide.editor.utils
...
/** Size of the reusable char buffer used to stream [io.github.rosemoe.sora.text.ContentLine]
 * contents out to disk. Mirrors ContentIO.CHAR_BUFFER_SIZE upstream. */
private const val WRITE_CHAR_BUFFER_SIZE = 1024
...
object ContentReadWrite {
    ...
    fun Content.writeTo(file: File, progressConsumer: ((Int) -> Unit)? = null) {
        ...
        file.writer().buffered(DEFAULT_BUFFER_SIZE * 2).use { writer ->
            ...
            var totalWrote = 0.0
            val charBuffer = CharArray(WRITE_CHAR_BUFFER_SIZE)
            try {
                for (lineIdx in 0..lastLine) {
                    val line = getLine(lineIdx)

                    // ContentLine no longer exposes a backing char[] directly (it may store text as
                    // Latin1 or UTF-16 packed bytes internally). Stream it out via getChars() in chunks
                    // instead, same approach as upstream ContentIO.writeTo().
                    var i = 0
                    while (i < line.length) {
                        val end = min(i + charBuffer.size, line.length)
                        line.getChars(i, end, charBuffer, 0)
                        writer.write(charBuffer, 0, end - i)
                        i = end
                    }
